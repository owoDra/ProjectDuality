...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DCAudioMixEffectsSubsystem)


//void UDCAudioMixEffectsSubsystem::Deinitialize()
//{
...
bool UDCAudioMixEffectsSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void UDCAudioMixEffectsSubsystem::PostInitialize()
{
	if (const UDCAudioSettings* DCAudioSettings = GetDefault<UDCAudioSettings>())
	{
		if (UObject* ObjPath = DCAudioSettings->DefaultControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Default Control Bus Mix reference missing from DC Audio Settings."));
			}
		}

		if (UObject* ObjPath = DCAudioSettings->LoadingScreenControlBusMix.TryLoad())
		{
			if (USoundControlBusMix* SoundControlBusMix = Cast<USoundControlBusMix>(ObjPath))
			{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Loading Screen Control Bus Mix reference missing from DC Audio Settings."));
			}
		}

		if (UObject* ObjPath = DCAudioSettings->UserSettingsControlBusMix.TryLoad())
		{
			if (USoundControlBusMix* SoundControlBusMix = Cast<USoundControlBusMix>(ObjPath))
			{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("User Control Bus Mix reference missing from DC Audio Settings."));
			}
		}

		if (UObject* ObjPath = DCAudioSettings->OverallVolumeControlBus.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from DC Audio Settings."));
			}
		}

		if (UObject* ObjPath = DCAudioSettings->MusicVolumeControlBus.TryLoad())
		{
			if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
			{
			...
			{
				ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from DC Audio Settings."));
			}
		}

		if (UObject* ObjPath = DCAudioSettings->SoundFXVolumeControlBus.TryLoad())
		{
			if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
			{
			...
			{
				ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from DC Audio Settings."));
			}
		}

		// Load HDR Submix Effect Chain
		for (const FDCSubmixEffectChainMap& SoftSubmixEffectChain : DCAudioSettings->HDRAudioSubmixEffectChain)
		{
			FDCAudioSubmixEffectsChain NewEffectChain;
		...
		for (const FDCSubmixEffectChainMap& SoftSubmixEffectChain : DCAudioSettings->LDRAudioSubmixEffectChain)
		{
			FDCAudioSubmixEffectsChain NewEffectChain;
...
void UDCAudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		if (const UDCGameSettings* DCGameSettings = GetDefault<UDCGameSettings>())
		{
			...
			if (UserMix)
			{
				...
				{
					const FSoundControlBusMixStage OverallControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, OverallControlBus, DCGameSettings->GetOverallVolume());
					const FSoundControlBusMixStage MusicControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, MusicControlBus, DCGameSettings->GetMusicVolume());
					const FSoundControlBusMixStage SoundFXControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, SoundFXControlBus, DCGameSettings->GetSoundFXVolume());
			...
			ApplyDynamicRangeEffectsChains(DCGameSettings->IsHDRAudioModeEnabled());
		}
	}
}

void UDCAudioMixEffectsSubsystem::ApplyDynamicRangeEffectsChains(bool bHDRAudio)
{
	TArray<FDCAudioSubmixEffectsChain> AudioSubmixEffectsChainToApply;
	TArray<FDCAudioSubmixEffectsChain> AudioSubmixEffectsChainToClear;
	...
	for (const FDCAudioSubmixEffectsChain& EffectChainToClear : AudioSubmixEffectsChainToClear)
	{
		bool bAddToList = true;

		for (const FDCAudioSubmixEffectsChain& SubmixEffectChain : AudioSubmixEffectsChainToApply)
		{
...
void UDCAudioMixEffectsSubsystem::OnLoadingScreenStatusChanged(bool bShowingLoadingScreen)
{
	ApplyOrRemoveLoadingScreenMix(bShowingLoadingScreen);
}

void UDCAudioMixEffectsSubsystem::ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix)
{
...
bool UDCAudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type World) const
{
